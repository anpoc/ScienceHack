            ...
            if i+3 < len(txt.splitlines()):
                for i2 in range(3):
                    ...
                    if t_match2:
                        return True
    return False
def type_check2(txt,type2check,main_page_types,line_lim):
    forcepass = False
    t_match_name = False
...
def isthismainpage2(page,main_page_types,line_lim=30):
    page_found = []
    text = page.get_text().encode("utf8") # get plain text (is in UTF-8)
    lines = text.splitlines()
    num_lines = len(lines)
    ...
    for type_it in (main_page_types):
        page_found.append(type_check2(text,type_it,main_page_types,line_lim))
...
def manual_4training_readPDF(pdf_path: str)-> List[int]:
    ...
    lines2check = 30
    for page in doc: # iterate the document pages
        main_page = isthismainpage2(page,main_page_types,lines2check)
        # print(main_page)
        if any(x > 0 for x in main_page):
            main_page_b = True
        else:
            main_page_b = False
        pages_test.append(main_page_b)
        NN_data.append(main_page)
    # pages_test=[float(b) for b in pages_test]
    # print(pages_test)
